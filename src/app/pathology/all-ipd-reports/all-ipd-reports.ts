...
@Component({
  selector: 'app-all-ipd-reports',
  standalone: false,
  templateUrl: './all-ipd-reports.html',
  styleUrl: './all-ipd-reports.css'
})
export class AllIpdReports implements OnInit {
  ...
  applyFilters(): void {
    ...
    this.filteredReports = this.allReports.filter(report => {
      // Enforce IPD-only results on this page
  ...
  private async fetchByReceiptNo(receipt: string): Promise<void> {
    try {
      this.isLoading = true;
      this.cdr.detectChanges();
      const url = `${environment.apiUrl}/pathology-reports?receipt=${encodeURIComponent(receipt)}&limit=${this.itemsPerPage}&enrich=false&patientType=IPD&_=${Date.now()}`;
      const resp = await firstValueFrom(this.http.get<any>(url));
